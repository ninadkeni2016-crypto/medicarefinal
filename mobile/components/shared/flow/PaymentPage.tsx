...
import api from '@/lib/api';   // ← was MISSING — caused payment save to fail
...
export default function PaymentPage({ appointment, onBack, clinicalData }: Props) {
    ...
    const invoiceId          = `INV-${String((appointment as any)._id || appointment.id).slice(-5).toUpperCase()}`;

    // Save payment status to backend & advance step
    const handleSuccess = async () => {
        ...
        try {
            ...
            toast({ title: '✅ Payment Successful!', description: `₹${finalTotal.toFixed(2)} recorded via ${selectedMethod}` });
            onBack();
        } catch (error: any) {
            const msg = error?.response?.data?.message || error?.message || 'Unknown error';
            console.error('Payment save failed:', msg);
            toast({ title: '❌ Payment save failed', description: msg });
        }
    };
    ...
    const initiatePayment = async () => {
        ...
        setTimeout(() => {
            ...
            if (selectedMethod === 'cash') {
                handleSuccess(); // Cash — mark immediately
            } else {
    ...
    const PaymentTimelineItem = ({
        title, desc, done, isLast = false, time = '',
    }: { title: string; desc: string; done: boolean; isLast?: boolean; time?: string }) => (
        ...
        return (
            <ScrollView style={{ flex: 1, backgroundColor: '#F8FAFC' }} contentContainerStyle={{ padding: 20, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16, marginBottom: 24 }}>
                    <TouchableOpacity onPress={onBack} style={{ width: 44, height: 44, borderRadius: 12, backgroundColor: '#FFFFFF', alignItems: 'center', justifyContent: 'center' }}>
                        <ArrowLeft size={20} color="#0F172A" />
                    </TouchableOpacity>
                    <View>
                        <Text style={{ fontSize: 20, fontWeight: '700', color: '#0F172A' }}>Payment Status</Text>
                ...
                {clinicalData.paymentDone ? (
                    <View style={{ backgroundColor: '#FFFFFF', borderRadius: 16, padding: 32, borderWidth: 1, borderColor: '#F1F5F9', alignItems: 'center' }}>
                        ...
                        <View style={{ width: '100%', height: 1, backgroundColor: '#E2E8F0', marginVertical: 24 }} />
                        <View style={{ width: '100%', gap: 12 }}>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                <Text style={{ color: '#64748B', fontSize: 13 }}>Invoice ID</Text>
                                <Text style={{ color: '#0F172A', fontWeight: '600', fontSize: 13 }}>{invoiceId}</Text>
                            </View>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                <Text style={{ color: '#64748B', fontSize: 13 }}>Date</Text>
                                <Text style={{ color: '#0F172A', fontWeight: '600', fontSize: 13 }}>{appointment.date}</Text>
                            </View>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                <Text style={{ color: '#64748B', fontSize: 13 }}>Method</Text>
                                <Text style={{ color: '#0F172A', fontWeight: '600', fontSize: 13, textTransform: 'capitalize' }}>{clinicalData.paymentMethod || 'Online'}</Text>
                            </View>
                        </View>
                    </View>
                ) : (
                    <View style={{ backgroundColor: '#FFFFFF', borderRadius: 16, padding: 32, borderWidth: 1, borderColor: '#F1F5F9', alignItems: 'center' }}>
                        ...
                        <Text style={{ fontSize: 15, color: '#64748B', marginTop: 8, textAlign: 'center', lineHeight: 22 }}>
                            Patient has been notified to pay ₹{finalTotal.toFixed(2)}.
                        </Text>
        ...
        return (
            <ScrollView style={{ flex: 1, backgroundColor: '#F8FAFC' }} contentContainerStyle={{ padding: 20, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16, marginBottom: 24 }}>
                    ...
                    <View>
                        <Text style={{ fontSize: 20, fontWeight: '700', color: '#0F172A' }}>Payment Receipt</Text>
                        <Text style={{ fontSize: 13, color: '#64748B', fontWeight: '500' }}>To {appointment.doctorName}</Text>
                    </View>
                </View>
                ...
                <View style={{ backgroundColor: '#FFFFFF', borderRadius: 16, padding: 32, borderWidth: 1, borderColor: '#F1F5F9', alignItems: 'center' }}>
                    ...
                    <View style={{ width: '100%', height: 1.5, backgroundColor: '#E2E8F0', marginVertical: 24 }} />
                    <View style={{ width: '100%', gap: 16, paddingHorizontal: 8 }}>
                        <PaymentTimelineItem title="Invoice Generated"     desc={`Bill ${invoiceId} created`}                      done={true} time="Step 4" />
                        <PaymentTimelineItem title="Payment Initiated"     desc="Secure gateway opened"                            done={true} time="Step 5" />
                        <PaymentTimelineItem title="Transaction Complete"  desc={`Paid via ${clinicalData.paymentMethod || 'Online'}`} done={true} isLast={true} />
                    </View>
                </View>
            </ScrollView>
        );
    ...
    return (
        <ScrollView style={{ flex: 1, backgroundColor: '#F8FAFC' }} contentContainerStyle={{ padding: 20, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16, marginBottom: 24 }}>
                ...
                <View>
                    <Text style={{ fontSize: 20, fontWeight: '700', color: '#0F172A' }}>Checkout</Text>
                    <Text style={{ fontSize: 13, color: '#64748B', fontWeight: '500' }}>{appointment.doctorName}</Text>
                </View>
            </View>

            {/* Bill Summary */}
            <View style={{ backgroundColor: '#FFFFFF', borderRadius: 16, padding: 20, marginBottom: 24, borderWidth: 1, borderColor: '#F1F5F9' }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                    <Text style={{ fontSize: 15, fontWeight: '700', color: '#0F172A' }}>Amount to Pay</Text>
                    <View style={{ backgroundColor: '#EFF6FF', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 12 }}>
                        <Text style={{ fontSize: 12, color: '#2563EB', fontWeight: '700' }}>{invoiceId}</Text>
                    </View>
                </View>
            ...
            <TouchableOpacity
                ...
                style={{ width: '100%', height: 56, borderRadius: 16, backgroundColor: loadingOrder ? '#94A3B8' : '#0F172A', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, elevation: 4 }}
            >
